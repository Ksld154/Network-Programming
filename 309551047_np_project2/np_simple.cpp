...
int forkProcess(vector<vector<string>> cmdTable, bool &line_end_with_pipeN, int &total_cmd_cnt) {
...
int npshell() {
    ...
    while(should_run) {
        ...
        // real command
            ...
            forkProcess(cmdTable, line_end_with_pipeN, total_cmd_cnt);
        }
...
int runServer(int port) {
    ...
    while(1) {
        ...
        socklen_t alen = sizeof(client_sin);

        // cout << "Waiting for connections..." << endl;
        int ssock = accept(msock, (struct sockaddr*)&client_sin, &alen);
        if(ssock < 0) {
